#!/usr/bin/env python3
"""Test script to verify the mixing matrix control in QuadHoverBraxEnv."""

import os
import jax
import jax.numpy as jnp
from train_brax_ppo import QuadHoverBraxEnv

def test_hover_mixing_control():
    """Test that QuadHoverBraxEnv correctly converts thrust+torques to motor commands."""
    ...
    print("=" * 70)
    print("Testing QuadHoverBraxEnv with Mixing Matrix Control")
    print("=" * 70)
    ...
    print(f"Target position: {env._target_pos}")
    print()
    ...
    print(f"Initial position (xyz): {state.pipeline_state.q[:3]}")
    print()
    ...
    test_cases = [
        ...
        ("Combined: thrust + roll", jnp.array([0.2, 0.2, 0.0, 0.0])),
    ]
    ...
    for test_name, action in test_cases:
        ...
        print(f"  Motor commands: [{motor_commands[0]:.3f}, {motor_commands[1]:.3f}, {motor_commands[2]:.3f}, {motor_commands[3]:.3f}] N")
        print()
    ...
    print("  ✓ QuadHoverBraxEnv ready for Brax PPO training!")
    print()
    print("Both environments now support mixing matrix control:")
    print("  - QuadHoverBraxEnv: Hover at fixed height")
    print("  - JaxMJXQuadBraxEnv: Follow random trajectories")

if __name__ == "__main__":
    test_hover_mixing_control()
