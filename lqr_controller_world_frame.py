...
PID_PLOTS_DIR = os.path.join(PLOTS_DIR, "lqr")
...
def plot_episode(data: dict, episode_num: int, save_dir: str = "plots"):
    ...
    fig.tight_layout()
    filepath = os.path.join(save_dir, f"lqr_episode_{episode_num}.png")
    ...
    fig2.tight_layout()
    filepath2 = os.path.join(save_dir, f"lqr_episode_{episode_num}_errors.png")
...
def evaluate(num_episodes: int = 5, render: bool = True, plot: bool = False,
    ...
    Args:
        ...
        min_voltage: Battery minimum voltage clamp (V).
        voltage_drop_base_per_sec: Base voltage drop rate (V/s).
        voltage_drop_load_per_sec: Load-dependent voltage drop gain (V/s).
    """
    # Load gains
    gains = DEFAULT_GAINS
    ...
    for ep in range(num_episodes):
        ...
        if "voltage" in info:
            print(f"  Voltage: {info['voltage']:.2f}V (scale={info.get('voltage_scale', 1.0):.3f})")
        ...
        if plot and len(ep_data["times"]) > 0:
            plot_episode(ep_data, ep + 1, save_dir="./plots/lqr")
    ...
    if plot and len(all_ep_data) > 0:
        summary_dir = os.path.join("./plots/lqr", "summary")
...
def main():
    ...
    parser.add_argument("--plot", action="store_true",
                        help="Save performance plots to ./plots/lqr/")
